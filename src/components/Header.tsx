        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-6">
              {navigationItems.map(item => <a key={item.name} href={item.href} className="flex items-center space-x-2 text-white hover:text-orange-100 transition-colors duration-200 relative">
                  <item.icon className="w-4 h-4" />
                  <span className="font-medium">{item.name}</span>
                  {item.badge && <Badge className="ml-1 bg-white text-orange-600 text-xs min-w-[18px] h-4 flex items-center justify-center p-0">
                      {item.badge}
                    </Badge>}
                </a>)}
            ...
            <div className="flex items-center gap-4">
              <Button size="sm" className="bg-white hover:bg-orange-100 text-orange-600 font-bold ml-4 h-9" onClick={scrollToRegistration}>
                Register Now
              </Button>
              
              {/* Mobile Menu Button */}
              <div className="md:hidden">
                <Button variant="ghost" size="sm" onClick={() => setIsSearchOpen(true)} className="mr-2 text-white hover:bg-orange-400">
          ...
          {/* Mobile Menu */}
          {isMenuOpen && <div className="md:hidden py-4 border-t border-orange-300">
              <nav className="flex flex-col space-y-4">
                {navigationItems.map(item => <a key={item.name} href={item.href} className="flex items-center justify-between text-white hover:text-orange-100 transition-colors duration-200" onClick={() => setIsMenuOpen(false)}>
                    <div className="flex items-center space-x-3">
                      <item.icon className="w-5 h-5" />
                      <span className="font-medium">{item.name}</span>
                    </div>
                    {item.badge && <Badge className="bg-white text-orange-600 text-xs">
                        {item.badge}
                      </Badge>}
                  </a>)}
                <div className="pt-4 border-t border-orange-300">
                  <Button className="bg-white hover:bg-orange-100 text-orange-600 w-full" onClick={scrollToRegistration}>
                    Register Now
                  </Button>
                </div>
              </nav>
